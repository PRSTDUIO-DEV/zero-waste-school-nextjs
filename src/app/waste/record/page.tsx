...
export default function WasteRecordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const pointsMessage = data.totalPoints ? ` คุณได้รับ ${data.totalPoints} คะแนน` : ''
        const badgeMessage = data.newBadges && data.newBadges.length > 0 
          ? ` 🏆 ได้รับเหรียญใหม่: ${data.newBadges.map((b: any) => b.name).join(', ')}` 
          : ''
  ...
  const getCategoryFromName = (name: string): string => {
    ...
    if (lowerName.includes('รีไซเคิล') || lowerName.includes('recycle')) {
      return 'รีไซเคิล'
    } else if (lowerName.includes('อินทรีย์') || lowerName.includes('organic')) {
      return 'อินทรีย์'
  ...
  const getFilteredWasteTypes = () => {
    ...
    return wasteTypes.filter(type => getCategoryFromName(type.name) === selectedCategory)
  }

  const getCategories = () => {
    const categories = [...new Set(wasteTypes.map(type => getCategoryFromName(type.name)))]
    return categories
  }
  ...
  const calculatePoints = () => {
    ...
    return Math.round(formData.weight * 1000 * Number(selectedType.pointFactor))
  }
        ...
        {success && (
          <div className="glass-card bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 mb-6 p-6 rounded-2xl">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-bold text-green-800 dark:text-green-200">สำเร็จ!</h3>
                <p className="text-green-700 dark:text-green-300 whitespace-pre-line">{success}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {error && (
          <div className="glass-card bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 mb-6 p-6 rounded-2xl">
            <div className="flex items-center">
              <div className="text-4xl mr-4">❌</div>
              <div>
                <h3 className="text-lg font-bold text-red-800 dark:text-red-200">เกิดข้อผิดพลาด</h3>
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="space-y-8">
            ...
            <div className="glass-card p-6">
              <h3 className="text-2xl font-bold text-gradient mb-4">เลือกประเภทขยะ</h3>
            ...
            <div className="glass-card">
              ...
              <div className="p-8">
                <form onSubmit={handleSubmit} className="space-y-8">
                      ...
                      {getFilteredWasteTypes().map(type => (
                        <div
                          ...
                          className={`glass-card p-4 cursor-pointer transition-all duration-300 hover:scale-105 ${
                            formData.wasteTypeId === type.id ? 'ring-4 ring-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : ''
                          }`}
                          onClick={() => setFormData(prev => ({ ...prev, wasteTypeId: type.id }))}
                        >
                          <div className="flex items-center space-x-4">
                            <div className="text-4xl">{getCategoryIcon(getCategoryFromName(type.name))}</div>
                            <div className="flex-1">
                              <h4 className="text-lg font-bold text-gradient">{type.name}</h4>
                  ...
                  {/* Weight Input */}
                  <div>
                    <label htmlFor="weight" className="form-label">น้ำหนัก (กิโลกรัม) *</label>
                    <input
                      id="weight"
                      type="number"
                      step="0.01"
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 text-center">
                      ใส่น้ำหนักเป็นกิโลกรัม (เช่น 1.5 กก.)
                    </p>
                  </div>
                  ...
                  {formData.wasteTypeId > 0 && formData.weight > 0 && (
                    <div className="glass-card p-6 text-center">
                      <h3 className="text-2xl font-bold text-gradient mb-4">คะแนนที่จะได้รับ</h3>
                      <div className="text-6xl font-bold text-gradient mb-4 animate-pulse-luxury">
                        {calculatePoints().toLocaleString()}
                      </div>
                      <p className="text-xl text-gray-600 dark:text-gray-300">คะแนน</p>
                      <div className="mt-4 text-sm text-gray-500 dark:text-gray-400">
                        {formData.weight} กก. × {Number(getSelectedWasteType()?.pointFactor)} คะแนน/กรัม × 1,000 = {calculatePoints().toLocaleString()} คะแนน
                      </div>
                    </div>
                  )}
                  ...
                  {/* Description */}
                  <div>
                    <label htmlFor="description" className="form-label">รายละเอียดเพิ่มเติม (ไม่บังคับ)</label>
                    <textarea
                      id="description"
                  ...
                  <div className="text-center">
                    <button
                      type="submit"
                      disabled={submitting || !formData.wasteTypeId || formData.weight <= 0}
            ...
            <div className="glass-card p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="glass-card p-6">
                  <h4 className="text-lg font-bold text-gradient mb-3">📏 การชั่งน้ำหนัก</h4>
                ...
                <div className="glass-card p-6">
                  <h4 className="text-lg font-bold text-gradient mb-3">♻️ การแยกขยะ</h4>
                  <ul className="space-y-2 text-gray-600 dark:text-gray-300">
            ...
            <div className="text-center">
              <div className="flex justify-center space-x-6">
                ...
                <Link href="/statistics" className="btn btn-glass px-8 py-4 text-lg">
                  <span className="mr-3">📊</span>
                  ดูสถิติ
                </Link>
              </div>
